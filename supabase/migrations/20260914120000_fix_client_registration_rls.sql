...
## Problem
The client registration flow inserts into the `clients` and `vehicles` tables
BEFORE the user is authenticated. The existing RLS INSERT policies on both
tables are scoped `TO authenticated` only, so an anonymous visitor trying to
register gets: "new row violates row-level security policy for table 'clients'".
...
2. Add `anon` SELECT policy scoped to `auth_user_id IS NULL` — needed because
   ...
   newly created row. Once the trigger sets `auth_user_id`, the row becomes
   invisible to anon.
